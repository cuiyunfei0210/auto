...
如果弹出 **Failed to load Python DLL / python312.dll / 找不到指定的模块**：这是旧包装（PyInstaller `_internal`）的报错。请删掉桌面上整个 `WallpaperStudio` 文件夹，再解压最新一次绿色勾的 `client-Windows`。新包 exe 旁边应有 `python312.dll` 和 `pythonw.exe`。不要只覆盖 exe。还不行就安装 [VC++ 运行库](https://aka.ms/vs/17/release/vc_redist.x64.exe)。
...
- 跳过二创，直接选本地文件夹上传
- 对接中转站 API（固定 `https://api.newxxt.top`，生图 `gpt-image-2`，起名可在对话 Key 可用模型里选）做图生图
...
「二创 API」固定走 `https://api.newxxt.top`：生图用 `gpt-image-2`，根据图片写标题用当前对话 Key 在中转站可用的模型（默认 `gpt-5.4-mini`，可在页面里刷新并切换）。OpenCode 配置里的 `gpt-5.x` 是对话模型，不要填进生图模型。
...
文件名提示词会走 `/v1/chat/completions`，并根据图片内容识图起名。标题模型从中转站 `GET /v1/models` 拉取，默认 `gpt-5.4-mini`。`gpt-image-2` 不能写标题。若下拉框只有一个模型，到 newxxt 后台给该 Key 分组开通更多对话模型后再点「刷新模型」。
