...
def add_mixed_indent_paragraph(doc, label, body):
    """Party line: label in bold 宋体, body in 仿宋."""
    paragraph = doc.add_paragraph()
    set_paragraph_format(paragraph, first_line_pt=INDENT_2_CHARS, line_pt=28)
    label_run = paragraph.add_run(label)
    set_run_font(label_run, size_pt=16, east_asia=EAST_ASIA_SONG, bold=True)
    body_run = paragraph.add_run(body)
    set_run_font(body_run, size_pt=16, east_asia=EAST_ASIA_BODY, bold=False)
    return paragraph
...
def add_page_number(doc):
    ...
    set_run_font(prefix, size_pt=10.5, east_asia=EAST_ASIA_SONG)
    ...
    set_run_font(page_run, size_pt=10.5, east_asia=EAST_ASIA_SONG)
    ...
    set_run_font(suffix, size_pt=10.5, east_asia=EAST_ASIA_SONG)
...
def build():
    ...
    set_run_font(title_run, size_pt=22, east_asia=EAST_ASIA_TITLE, bold=True)
    ...
    items = [
        "一、请求贵院对（2025）冀0108执4502号执行案件恢复强制执行，强制二被申请人立即履行河北省石家庄市裕华区人民法院（2025）冀0108民初4406号民事调解书确定的给付义务，向申请人支付剩余工资款人民币43000元及相应利息、诉讼费用625元。",
    ...
    facts = [
        ...
        "案件执行过程中，在贵院主持下，申请人与二被申请人达成执行和解协议，约定二被申请人于协议确定的履行期限内清偿全部剩余债务。基于二被申请人作出的还款承诺，申请人向贵院申请解除了对二被申请人的限制高消费措施。此后，本案以执行和解方式结案并已归档。",
        "现执行和解协议约定的履行期限已经届满，二被申请人无正当理由拒不履行和解协议确定的还款义务，仅以“没钱”为由推诿拖延，有隐匿、转移财产的重大嫌疑。",
